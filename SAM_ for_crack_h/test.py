...
def inference(args, multimask_output, db_config, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f iou %f' % (
            i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2]))
    metric_list = metric_list / len(db_test)
    for i in range(1, args.num_classes + 1):
        try:
            logging.info('Mean class %d name %s mean_dice %f mean_hd95 %f iou %f' % (i, class_to_name[i], metric_list[i - 1][0], metric_list[i - 1][1], metric_list[i - 1][2]))
        except:
            logging.info('Mean class %d mean_dice %f mean_hd95 %f iou %f' % (i, metric_list[i - 1][0], metric_list[i - 1][1], metric_list[i - 1][2]))
    performance = np.mean(metric_list, axis=0)[0]
    mean_hd95 = np.mean(metric_list, axis=0)[1]
    iou = np.mean(metric_list, axis=0)[2]
    logging.info(f'Testing performance in best val model: mean_dice : {performance} mean_hd95 : {mean_hd95} iou : {iou}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='/home/lifeiyang/SAMed_fortest/checkpoints/sam_vit_h_4b8939.pth',
                        help='Pretrained checkpoint')
    parser.add_argument('--lora_ckpt', type=str, default='/home/lifeiyang/SAMed_fortest/checkpoints/epoch_29.pth', help='The checkpoint from LoRA')
